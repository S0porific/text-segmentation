...
def main(args):
    ...
    for dataset_path in dataset_folders:
        ...
        if args.wiki:
            if args.wiki_folder:
                dataset = WikipediaDataSet(dataset_path, word2vec, folder=True,
                                           high_granularity=False)
            else:
                dataset = WikipediaDataSet(dataset_path, word2vec,
                                           high_granularity=False)
        else:
            dataset = ChoiDataset(dataset_path, word2vec)

        dl = DataLoader(dataset, batch_size=args.bs, collate_fn=collate_fn,
                        shuffle=False)
        ...
        with tqdm(desc='Testing', total=len(dl)) as pbar:
            ...
            for i, (data, targets, paths) in enumerate(dl):
                ...
                pbar.update()
                output = model(data)
                targets_var = Variable(
                    maybe_cuda(torch.cat(targets, 0), args.cuda),
                    requires_grad=False)
                batch_loss = 0
                ...
                for k, t in enumerate(targets):
                    ...
                    sentences_length = [s.size()[0] for s in
                                        data[k]] if args.calc_word else None
                    to_idx = int(current_target_idx + document_sentence_count)
                    h = output_seg[current_target_idx: to_idx]

                    # hypothesis and targets are missing classification of
                    # last sentence, and therefore we will add 1 for both
                    h = np.append(h, [1])
