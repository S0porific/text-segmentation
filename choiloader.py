...
import numpy as np
import random
from text_manipulation import split_sentences, word_model, \
    extract_sentence_words
import utils
import math
    ...
    for data, targets, path in batch:
        try:
            ...
            for curr_sentence_index in range(0, len(data)):
                from_index = max(
                    [0, curr_sentence_index - before_sentence_count])
                to_index = min(
                    [curr_sentence_index + after_sentence_count + 1, max_index])
                sentences_window = [word for sentence in
                                    data[from_index:to_index] for word in
                                    sentence]
                tensored_data.append(
                    torch.FloatTensor(np.concatenate(sentences_window)))
...
def clean_paragraph(paragraph):
    cleaned_paragraph = paragraph.replace("'' ", " ").replace(" 's",
                                                              "'s").replace(
        "``", "").strip('\n')
    return cleaned_paragraph


def read_choi_file(path, word2vec, train, return_w2v_tensors=True,
                   manifesto=False):
    ...
    for paragraph in paragraphs:
        ...
        if sentences:
            sentences_count = 0
            # This is the number of sentences in the paragraph and where we
            # need to split.
...
# Returns a list of batch_size that contains a list of sentences, where each
# word is encoded using word2vec.
class ChoiDataset(Dataset):
    def __init__(self, root, word2vec, train=False, folder=False,
                 manifesto=False, folders_paths=None):
        ...
        if len(self.textfiles) == 0:
            raise RuntimeError(
                'Found 0 images in subfolders of: {}'.format(root))
        self.train = train
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        return read_choi_file(path, self.word2vec, self.train,
