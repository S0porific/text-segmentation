...
wikipedia_namespaces = ['Category', 'File', 'Ru', 'Wikipedia', 'Talk', 'User',
                        'MediaWiki', 'Template', 'Help', 'Portal', 'Book',
                        'Draft',
                        'Education Program', 'TimedText', 'Module', 'Gadget',
                        'Gadget definition', 'Media', 'Special']
...
def process_header(header):
    ...
    not_valid = title.isdigit() or any(
        title.startswith(prefix + ':' or prefix + ' talk:') for prefix in
        wikipedia_namespaces) or title.endswith(disambigutaiton_pattern)
...
def process_section(section, id):
    ...
    for sentence in sentences:
        is_list_sentence = wiki_utils.get_list_token() + "." == sentence.encode(
            'utf-8')
        if '\n' in sentence:
            logger.info(
                "DocId: " + str(id) + "   back slash in sentence: " + sentence)
        if (wiki_utils.get_list_token() in sentence) and (
            ...
            logger.info(
                "DocId: " + str(id) + "     Special case 1: " + sentence)
            continue
    ...
    if len(re.findall('[a-zA-Z]',
                      section_text)) < wiki_thresholds.min_section_char_count:
        valid_section = False
...
def is_valid_article(valid_section_count, section_count):
    if valid_section_count < wiki_thresholds.min_valid_section_count:
        return False, "Valid section count is too low: " + str(
            valid_section_count)
    ...
    if valid_section_percentage < wiki_thresholds.min_valid_section_percentage:
        return False, "Valid section percentage is too low: " + str(
...
def process_content(content, id):
    ...
    for i in range(len(sections_with_headers)):
        ...
        if wiki_utils.is_seperator_line(section):
            ...
            is_valid_section, section_sentences, message = process_section(
                section, id)
            section_count += 1
            if (is_valid_section):
                ...
                logger.info('Invalid section in article id: ' + id +
                            '    Reason: ' + message + '    Content: ' + vec_to_text(
                    section_sentences).strp('\n'))
...
def process_article(article):
    ...
    if not is_valid_header:
        logger.info(
            'Invalid header in doc id: ' + str(id) + '     header:   ' + header)
        return "", id, False
    ...
    if not (is_valid_content):
        logger.info(
            'Invalid article in doc id: ' + str(id) + '.  ' + debug + '\n\n')
    else:
...
def process_wiki_file(path, output_folder, train_ratio, test_ratio,
                      forbidden_train_ids):
    train_size = 0
    dev_size = 0
    test_size = 0
    ...
    articles = [s for s in raw_content.strip().split(doc_split_delimiter) if
                len(s) > 0]
    ...
    for article in articles:
        ...
        if (
                random_num > train_ratio and random_num <= train_ratio + test_ratio) or int(
                id) in forbidden_train_ids:
            partition = "test"
            test_size += 1
            ...
            partition = "train"
            train_size += 1
        output_sub_folder = os.path.join(output_folder, partition,
                                         get_file_path(id))
...
def get_forbidden_train_ids():
    ...
    forbidden_train_ids = []
    for k, v in wiki_cities.items():
        forbidden_train_ids.append(int(v))
    for k, v in wiki_elements.items():
        forbidden_train_ids.append(int(v))
...
def process_wiki_folder(input_folder, output_folder, train_ratio, test_ratio):
    ...
    total_dev_size = 0
    total_test_size = 0
    folders = [o for o in os.listdir(input_folder) if
               os.path.isdir(os.path.join(input_folder, o))]
    ...
    for folder in folders:
        ...
        for file in files:
            created_articles, processed_articles, train_size, dev_size, test_size = process_wiki_file(
                file, output_folder, float(train_ratio), float(test_ratio),
                forbidden_train_ids)
            ...
            if (total_created_articles - previous_debug > 2500):
                ...
                print('Created ' + str(
                    total_created_articles) + ' wiki articles, out of ' + str(
                    total_processed_articles) + ' processed articles')
    total_samples = total_train_size + total_dev_size + total_test_size
    print('total_samples = ', str(total_samples))
    print("#train = ", total_train_size, "ratio: ",
          "{:.2f}".format(total_train_size / float(total_samples)))
    print("#dev = ", total_dev_size, "ratio: ",
          "{:.2f}".format(total_dev_size / float(total_samples)))
    print("#test = ", total_test_size, "ratio: ",
          "{:.2f}".format(total_test_size / float(total_samples)))
...
def trainTestDev(destFolder, train_size, test_size):
    ...
    if not os.path.exists(destFolder):
        ...
        return
    folders = [o for o in os.listdir(destFolder) if
               os.path.isdir(os.path.join(destFolder, o))]
    for folder in folders:
...
def main(args):
    ...
    cmd = ['python', str(Path(__file__).parent / 'wiki_extractor.py'), '-s',
           '-o', args.temp, '--article_count', str(args.article_count),
           '--lists']
    print(cmd)
    ...
    print("avg len sentence = " + str(
        sum_sentneces_for_avg / float(num_sentneces_for_avg)))
    print('done')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--processes',
                        help='Num of processors to use in wiki_extractor')
    parser.add_argument('--no_extractor', help='Skip wiki-extractor',
                        action='store_true')
    parser.add_argument('--temp', help='folder to save temporal files',
                        required=True)
    ...
    parser.add_argument("--article_count",
                        help='max number of wikipedia articles to extract',
                        default=1000000)
